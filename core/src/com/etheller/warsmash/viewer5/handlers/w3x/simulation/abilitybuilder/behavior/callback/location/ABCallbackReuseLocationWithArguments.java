...
public class ABCallbackReuseLocationWithArguments extends ABLocationCallback {

	private ABStringCallback name;
	private Map<String, ABCallback> arguments;

	@Override
	public AbilityPointTarget callback(final CUnit caster, final ABLocalDataStore localStore,
			final int castId) {
		...
		if (config.getReuseCallbacks() != null) {
			...
			if (callback != null && callback instanceof ABLocationCallback) {
				...
				return ((ABLocationCallback) callback).callback(caster, localStore, castId);
			} else {
