...
public class ABCallbackArgumentBoolean extends ABCondition {
	...
	public Boolean callback(final CSimulation game, final CUnit caster, final LocalDataStore localStore,
		...
		if (cbck != null && cbck instanceof ABBooleanCallback) {
			return ((ABBooleanCallback) cbck).callback(game, caster, localStore, castId);
		} else {
			System.err.println(
					"Trying to run ReuseBooleanCallback, but key is missing or callback was the wrong type: " + keyS);
		}
		return null;
	}
