...
public class ABCallbackReuseWidget extends ABWidgetCallback {
	...
	public CWidget callback(final CSimulation game, final CUnit caster, final LocalDataStore localStore,
		...
		if (config.getReuseCallbacks() != null) {
			...
			if (callback != null && callback instanceof ABWidgetCallback) {
				return ((ABWidgetCallback) callback).callback(game, caster, localStore, castId);
			} else {
				System.err.println("Trying to run ReuseWidgetCallback, but key is missing or callback was the wrong type: " + keyS);
			}
		} else {
			System.err.println("Trying to run ReuseWidgetCallback, but none defined");
		}
		return null;
	}
