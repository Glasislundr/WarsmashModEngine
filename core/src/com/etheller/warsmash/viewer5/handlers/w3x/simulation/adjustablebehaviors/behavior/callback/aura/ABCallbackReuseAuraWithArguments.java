...
public class ABCallbackReuseAuraWithArguments extends ABAuraCallback {

	private ABStringCallback name;
	private Map<String, ABCallback> arguments;

	@Override
	public ABAuraEffect callback(final CUnit caster, final ABLocalDataStore localStore, final int castId) {
		...
		if (config.getReuseCallbacks() != null) {
			...
			if (callback != null && callback instanceof ABAuraCallback) {
				...
				return ((ABAuraCallback) callback).callback(caster, localStore, castId);
			} else {
				System.err.println(
						"Trying to run ReuseAuraCallback, but key is missing or callback was the wrong type: " + keyS);
			}
		} else {
			System.err.println("Trying to run ReuseAuraCallback, but none defined");
		}
		return null;
	}
