...
import com.etheller.warsmash.viewer5.handlers.w3x.simulation.abilitybuilder.behavior.ABBehaviorSendOrder;
...
public class ABAbilityBuilderActivePairing extends ABAbilityBuilderGenericActive
	...
	public ABAbilityBuilderActivePairing(int handleId, War3ID code, War3ID alias,
...
//				System.err.println(caster.getUnitType().getName() + " Have final partner, issuing self order behavior");
				if (isOffId) {
					return new ABBehaviorSendOrder(caster, this, this.getPairOffOrderId(game, caster),
							this.getBaseOrderId(), finalPartner);
				}
				return new ABBehaviorSendOrder(caster, this, this.getPairOrderId(game, caster), this.getBaseOrderId(),
						finalPartner);
			} else {
				return null;
			}
