...
public class ABCallbackReuseIDWithArguments extends ABIDCallback {
	...
	public War3ID callback(final CSimulation game, final CUnit caster,
		...
		if (config.getReuseCallbacks() != null) {
			...
			if (callback != null && callback instanceof ABIDCallback) {
				...
				return ((ABIDCallback) callback).callback(game, caster, localStore, castId);
			} else {
				System.err.println("Trying to run ReuseIDCallback, but key is missing or callback was the wrong type: " + keyS);
			}
		} else {
			System.err.println("Trying to run ReuseIDCallback, but none defined");
		}
		return null;
	}
