...
import com.etheller.warsmash.viewer5.handlers.w3x.simulation.abilitybuilder.types.impl.ABAbilityBuilderAbilityType;
...
public class ABAbilityBuilderTypeDefinition
	...
	public ABAbilityBuilderTypeDefinition(ABAbilityBuilderConfiguration abilityBuilderParser) {
		super();
	...
	public CAbilityType<?> createAbilityType(final War3ID alias, final GameObject abilityEditorData) {
		if (abilityEditorData != null) {
			...
			final List<ABAbilityBuilderAbilityTypeLevelData> levelData = new ArrayList<>();
	...
	@Override
	protected ABAbilityBuilderAbilityTypeLevelData createLevelData(final GameObject abilityEditorData,
			final int level) {
		...
		return new ABAbilityBuilderAbilityTypeLevelData(targetsAllowedAtLevel, area, castRange, castTime, cooldown,
	...
	protected CAbilityType<?> innerCreateAbilityType(final War3ID alias, final GameObject abilityEditorData,
			final List<ABAbilityBuilderAbilityTypeLevelData> levelData) {
		if (abilityEditorData != null) {
			return new ABAbilityBuilderAbilityType(alias, abilityEditorData.getFieldAsWar3ID(CODE, -1), abilityEditorData, levelData, parser);
		} else {
			return new ABAbilityBuilderAbilityType(alias, alias, null, null, parser);
		}
	}
