...
public class ABActionCreateTimedTickingPausedBuff implements ABSingleAction {
	...
	public void runAction(final CUnit caster, final ABLocalDataStore localStore, final int castId) {
		...
		if (showIcon != null) {
			ability = new ABTimedTickingPausedBuff(localStore.game.getHandleIdAllocator().createId(),
		...
		if (uniqueValues != null) {
			for (ABStringCallback key : uniqueValues.keySet()) {
				ability.addUniqueValue(uniqueValues.get(key).callback(caster, localStore, castId),
