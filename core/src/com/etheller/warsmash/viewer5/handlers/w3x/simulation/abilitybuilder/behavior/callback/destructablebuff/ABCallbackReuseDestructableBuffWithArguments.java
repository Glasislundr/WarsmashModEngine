...
public class ABCallbackReuseDestructableBuffWithArguments extends ABDestructableBuffCallback {

	private ABStringCallback name;
	private Map<String, ABCallback> arguments;

	@Override
	public CDestructableBuff callback(final CUnit caster, final LocalDataStore localStore, final int castId) {
		...
		if (config.getReuseCallbacks() != null) {
			...
			if (callback != null && callback instanceof ABDestructableBuffCallback) {
				...
				return ((ABDestructableBuffCallback) callback).callback(caster, localStore, castId);
			} else {
				System.err.println(
						"Trying to run ReuseDestructableBuffCallback, but key is missing or callback was the wrong type: "
								+ keyS);
			}
		} else {
