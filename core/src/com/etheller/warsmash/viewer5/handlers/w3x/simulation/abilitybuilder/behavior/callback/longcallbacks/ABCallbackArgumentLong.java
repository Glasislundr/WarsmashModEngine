...
public class ABCallbackArgumentLong extends ABLongCallback {
	...
	public Long callback(final CSimulation game, final CUnit caster,
		...
		if (cbck != null && cbck instanceof ABLongCallback) {
			return ((ABLongCallback) cbck).callback(game, caster, localStore, castId);
		} else {
			System.err.println("Trying to run ReuseLongCallback, but key is missing or callback was the wrong type: " + keyS);
		}
		return null;
	}
