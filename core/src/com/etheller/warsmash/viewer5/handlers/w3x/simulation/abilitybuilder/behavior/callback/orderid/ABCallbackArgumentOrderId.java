...
public class ABCallbackArgumentOrderId extends ABOrderIdCallback {
	...
	public Integer callback(final CSimulation game, final CUnit caster, final LocalDataStore localStore,
		...
		if (cbck != null && cbck instanceof ABOrderIdCallback) {
			return ((ABOrderIdCallback) cbck).callback(game, caster, localStore, castId);
		} else {
			System.err.println(
					"Trying to run ReuseOrderIdCallback, but key is missing or callback was the wrong type: " + keyS);
		}
		return null;
	}
