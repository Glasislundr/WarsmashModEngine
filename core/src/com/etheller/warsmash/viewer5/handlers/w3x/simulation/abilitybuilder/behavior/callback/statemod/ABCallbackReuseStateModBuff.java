...
public class ABCallbackReuseStateModBuff extends ABStateModBuffCallback {

	private ABStringCallback name;

	@Override
	public StateModBuff callback(final CUnit caster, final ABLocalDataStore localStore,
			final int castId) {
		...
		if (config.getReuseCallbacks() != null) {
			...
			if (callback != null && callback instanceof ABStateModBuffCallback) {
				return ((ABStateModBuffCallback) callback).callback(caster, localStore, castId);
			} else {
